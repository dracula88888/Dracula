...
public class DiamondLightningPlugin extends JavaPlugin implements Listener {
    ...
    // true = ability activated, waiting for hit
    private final Map<UUID, Boolean>    abilityActive  = new HashMap<>();
    // ms timestamp when cooldown started
    ...
    // YELLOW color used everywhere for the sword theme
    ...
    private static final ChatColor B  = ChatColor.BOLD;
    ...
    private void giveLightningSword(Player player) {
        ...
        // Sword name: YELLOW + BOLD, sparkling ⚡
        meta.setDisplayName(Y + "" + B + "⚡ The Last Storm");
        List<String> lore = new ArrayList<>();
        lore.add(G + "Forged in the storms of AltarSMP");
        lore.add("");
        lore.add(Y + "Passive: " + W + "Looting IV · Unbreakable");
        lore.add(Y + "Ability: " + W + "Lightning Strike");
    ...
    // ── Glint on sword switch (brief shimmer, then disappears) ───────────────

    private void startIdleGlint(Player player) {
        UUID id = player.getUniqueId();
        if (glintTasks.containsKey(id)) return;
        // Yellow shimmer frames, then gone
        String[] frames = {
            G + "⚡ The Last Storm",
            Y + "⚡ The Last Storm",
            Y + "" + B + "⚡ The Last Storm",
    ...
    public void onSneak(org.bukkit.event.player.PlayerToggleSneakEvent event) {
        ...
        // 8s window boss bar: YELLOW, drains 1→0
        new BukkitRunnable() {
            int secondsLeft = WINDOW_SECONDS;
            @Override public void run() {
                if (!player.isOnline()) { cancel(); return; }
                if (!abilityActive.getOrDefault(id, false)) {
                    hideBar(player); cancel(); return;
                }
                double progress = (double) secondsLeft / WINDOW_SECONDS;
                BarColor color = secondsLeft <= 2 ? BarColor.RED : BarColor.YELLOW;
                showBar(player,
                    Y + "⚡ " + W + "Strike!  " + G + secondsLeft + "s",
                    color, progress);
                secondsLeft--;
            }
        }.runTaskTimer(this, 0L, 20L);

        // 8s expiry — deactivate, then START COOLDOWN (18s)
        BukkitTask expiry = new BukkitRunnable() {
            @Override public void run() {
                ...
                clearActionBar(player);
                // 8s passed without hit → start 18s cooldown
    ...
    // ── 18s Cooldown bar ─────────────────────────────────────────────────────
    // YELLOW, SEGMENTED_20, 0→1 fills, title: "⚡ Cooldown  Xs"
        ...
        new BukkitRunnable() {
            ...
            @Override public void run() {
                ...
                if (secondsLeft == 0) {
                    cooldownStart.remove(id);
                    showBar(player,
                        Y + "" + B + "⚡ " + W + "The Last Storm  " + ChatColor.GREEN + "✔ READY",
                        BarColor.GREEN, 1.0);
                    cancel();
                    // No action bar text — nothing on screen after cooldown
                    return;
                }
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        // Death message: YELLOW + BOLD prefix
        String msg = event.getDeathMessage();
        if (msg != null) event.setDeathMessage(Y + "" + B + "⚡ " + ChatColor.RESET + msg);
        ...
        for (int i = 0; i < 100; i++) {
            ...
            // Alternate bright yellow and gold tones
            Color c = (i % 3 == 0) ? Color.fromRGB(255, 215, 0)   // gold
                    : (i % 3 == 1) ? Color.fromRGB(255, 255, 0)   // yellow
                    :                Color.fromRGB(255, 165, 0);   // orange-yellow
