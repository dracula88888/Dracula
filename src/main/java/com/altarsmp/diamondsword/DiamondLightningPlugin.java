...
public class DiamondLightningPlugin extends JavaPlugin implements Listener {
    ...
    public void giveLightningSword(Player player) {
        ...
        // Unbreakable Mending-ის ზემოთ — enchant list order:
        // Sharp V, Sweeping III, Looting IV, Unbreaking III (Unbreakable tooltip), Mending
    ...
    public void onSneak(org.bukkit.event.player.PlayerToggleSneakEvent event) {
        ...
        // 8 წამიანი გაფრთხილება action bar-ში ზემოთ
    ...
    private void spawnActivationCircle(Player player) {
        ...
        double radius = 8.0;
        int points = 120;
        int layers = 4; // სქელი წრე

        new BukkitRunnable() {
            int tick = 0;
            final int maxTicks = 60; // 3 წამი (20 tick * 3)
