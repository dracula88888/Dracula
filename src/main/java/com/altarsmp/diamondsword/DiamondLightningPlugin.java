...
public class DiamondLightningPlugin extends JavaPlugin implements Listener {
    ...
    public void onSneak(org.bukkit.event.player.PlayerToggleSneakEvent event) {
        ...
        // 8 წამიანი გაფრთხილება — action bar-ში ზემოთ, თეთრი
    ...
    // 8 წამი countdown — action bar-ში, თეთრი სტილი
    private void startWindowCountdown(Player player, UUID id) {
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                // bar ივსება მარჯვნიდან მარცხნივ (დრო იწურება)
                int bars = 16;
                ...
                String msg =
                    ChatColor.WHITE + "⚡ Lightning Strike " +
                    ChatColor.DARK_GRAY + "| " +
                    ChatColor.WHITE + bar +
                    ChatColor.DARK_GRAY + " | " +
    ...
    public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
        ...
        // knockback გამორთული — velocity reset target-ზე
Bukkit.getScheduler().runTaskLater(this, () -> target.setVelocity(new org.bukkit.util.Vector(0, 0, 0)), 1L);
    ...
    // killing animation — თეთრი thunder სტილი
    @EventHandler
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        world.spawnParticle(Particle.ELECTRIC_SPARK, loc.clone().add(0, 1, 0), 50, 0.5, 1, 0.5, 0.5);
            ...
            public void run() {
                ...
                for (int i = 0; i < points; i++) {
                    ...
                    for (int layer = 0; layer < layers; layer++) {
                        double y = center.getY() + (layer * 0.15);
