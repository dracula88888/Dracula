...
public class DiamondLightningPlugin extends JavaPlugin implements Listener {
    ...
    private void giveLightningSword(Player player) {
        ...
        meta.setDisplayName(ChatColor.YELLOW + "" + ChatColor.BOLD + "->š¡ The Last Storm");
        List<String> lore = new ArrayList<>();
        lore.add(ChatColor.GRAY + "Forged in the storms of AltarSMP");
        lore.add("");
        lore.add(ChatColor.YELLOW + "Passive: " + ChatColor.WHITE + "Looting IV Â· Unbreakable");
        lore.add(ChatColor.YELLOW + "Ability: " + ChatColor.WHITE + "Lightning Strike");
        lore.add(ChatColor.DARK_GRAY + "Sneak ->->-> activate  |  Hit ->->-> ->š¡ Strike");
        lore.add(ChatColor.DARK_GRAY + "Window: " + WINDOW_SECONDS + "s  Â·  Cooldown: " + COOLDOWN_SECONDS + "s");
    ...
    private void startIdleGlint(Player player) {
        ...
        String[] frames = {
            ChatColor.GOLD   + "->š¡ The Last Storm",
            ChatColor.YELLOW + "" + ChatColor.BOLD + "->š¡ The Last Storm",
            ChatColor.YELLOW + "->š¡ The Last Storm",
            ChatColor.GOLD   + "->š¡ The Last Storm",
            ""  // final frame = clear
        };
    ...
    public void onSneak(org.bukkit.event.player.PlayerToggleSneakEvent event) {
        ...
        new BukkitRunnable() {
            ...
            @Override public void run() {
                ...
                showBar(player,
                    ChatColor.YELLOW + "->š¡ " + ChatColor.WHITE + "Strike!  " + ChatColor.GRAY + secondsLeft + "s",
                    color, progress);
                secondsLeft--;
            }
        ...
        new BukkitRunnable() {
            ...
            @Override public void run() {
                ...
                if (secondsLeft == 0) {
                    ...
                    showBar(player,
                        ChatColor.YELLOW + "" + ChatColor.BOLD + "->š¡ The Last Storm  " + ChatColor.GOLD + "READY",
                        BarColor.YELLOW, 1.0);
                    cancel();
                    return;
                }
                ...
                showBar(player,
                    ChatColor.YELLOW + "->š¡ " + ChatColor.WHITE + "Cooldown  " + ChatColor.GRAY + secondsLeft + "s",
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        if (msg != null)
            event.setDeathMessage(ChatColor.YELLOW + "" + ChatColor.BOLD + "->š¡ " + ChatColor.RESET + msg);
