...
public class WrathOfThunder extends JavaPlugin implements Listener, CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (ownerUUID == null) {
            if (!player.getName().equals("Vlad_Dracula888")) {
                player.sendMessage(ChatColor.RED + "✗ This sword is not meant for you.");
                return true;
            }
            ownerUUID = player.getUniqueId();
            getConfig().set("owner-uuid", ownerUUID.toString());
            saveConfig();
            player.sendMessage(ChatColor.WHITE + "" + ChatColor.BOLD + "You are now the eternal owner of the Wrath of Thunder");
        }
        ...
        String bar = ChatColor.GOLD + "" + ChatColor.BOLD + "⚡ ═══════════════════════════════ ⚡";
        Bukkit.broadcastMessage(bar);
        Bukkit.broadcastMessage(
            ChatColor.GRAY + "A " +
            ChatColor.WHITE + "" + ChatColor.BOLD + "Legendary" +
            ChatColor.GRAY + " artifact has entered the world:"
        );
        Bukkit.broadcastMessage(
            ChatColor.WHITE + "" + ChatColor.BOLD + "  ⚡ Wrath of Thunder"
        );
        Bukkit.broadcastMessage(
            ChatColor.GRAY + "  Claimed by: " +
            ChatColor.RED + "" + ChatColor.BOLD + player.getName()
        );
        Bukkit.broadcastMessage(
