...
public class WrathOfThunder extends JavaPlugin implements Listener, CommandExecutor {
    ...
    // ── PDC key: sword identity tag (survives any rename) ──
    private NamespacedKey swordKey;

    // ── owner UUID (loaded from config; set on first /wrath use) ──
    private UUID ownerUUID = null;

    // ── tracks whether the sword currently exists in the world ──
    private boolean swordExists = false;

    @Override
    public void onEnable() {
        saveDefaultConfig();
        swordKey = new NamespacedKey(this, "wrath_of_thunder");
    ...
    // ── helper: does the owner currently have the sword in their inventory? ──
    private boolean ownerHasSword() {
        if (ownerUUID == null) return false;
        Player owner = Bukkit.getPlayer(ownerUUID);
        if (owner == null || !owner.isOnline()) return false;
        return Arrays.stream(owner.getInventory().getContents()).anyMatch(this::isWrathOfThunder);
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        // ── ONE sword rule: refuse if sword already exists anywhere ──
        if (swordExists) {
            player.sendMessage(ChatColor.RED + "✗ The Wrath of Thunder already exists in this world. There can only be one.");
            return true;
        }
        ...
        logSwordClaim(player);

        // ── server-wide legendary broadcast ──
        String broadcast =
            ChatColor.DARK_GRAY + "[" +
            ChatColor.WHITE + "" + ChatColor.BOLD + "⚡ Wrath of Thunder" +
            ChatColor.DARK_GRAY + "] " +
            ChatColor.YELLOW + player.getName() +
            ChatColor.GRAY + " has claimed the " +
            ChatColor.WHITE + "" + ChatColor.BOLD + "Legendary Sword" +
            ChatColor.GRAY + "!";
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        if (isOwner(dead)) {
            ...
            // swordExists stays true — owner gets it back on respawn
        }
    ...
    public void onPlayerRespawn(PlayerRespawnEvent event) {
        ...
        Bukkit.getScheduler().runTaskLater(this, () -> {
            ...
            if (!hasSword) {
                // swordExists is already true from before death — just restore it
                player.getInventory().addItem(createWrathOfThunder());
    ...
    // ── log sword claim to file ──
    private void logSwordClaim(Player player) {
        File logFile = new File(getDataFolder(), "claims.log");
        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
        String line = "[" + timestamp + "] " + player.getName() + " (" + player.getUniqueId() + ") claimed the Wrath of Thunder";
        ...
        try {
            ...
            getLogger().warning("Failed to write to claims.log: " + e.getMessage());
        }
    }

    // ── checks: PDC tag — survives any display name change ──
    private boolean isWrathOfThunder(ItemStack item) {
        ...
        return pdc.has(swordKey, PersistentDataType.BYTE);
    }

    // ── creates the sword and stamps the PDC tag on it ──
    public ItemStack createWrathOfThunder() {
